...
## Code for the gRNA skipping analysis shown in Figure 6C (KREH1 OE for CYb)
...
KREH1OECYb <- read.csv(file.choose(), sep = ",", header=TRUE)

# Remove pre-edited reads
KREH1OECYbnopre <- KREH1OECYb %>% filter(!(junc_len==0&edit_stop==558))

# Subset reads with ESS 24
KREH1OECYb_558 <- KREH1OECYbnopre %>% filter(edit_stop==558)

# Perform the junction analysis each sample individually
OEUN_WT1 <- KREH1OECYb_558 %>% filter(sample=="REH1_MinTet_Rep1_CYb_OE")
OEUN_WT2 <- KREH1OECYb_558 %>% filter(sample=="REH1_MinTet_Rep2_CYb_OE")
OEUN_KA1 <- KREH1OECYb_558 %>% filter(sample=="REH1_MinTet_Rep1_CYb_M1")
OEUN_KA2 <- KREH1OECYb_558 %>% filter(sample=="REH1_MinTet_Rep2_CYb_M1")
OEUN_EQ1 <- KREH1OECYb_558 %>% filter(sample=="REH1_MinTet_Rep1_CYb_E1")
OEUN_EQ2 <- KREH1OECYb_558 %>% filter(sample=="REH1_MinTet_Rep2_CYb_E1")

OEIN_WT1 <- KREH1OECYb_558 %>% filter(sample=="REH1_PlusTet_Rep1_CYb_OE")
OEIN_WT2 <- KREH1OECYb_558 %>% filter(sample=="REH1_PlusTet_Rep2_CYb_OE")
OEIN_KA1 <- KREH1OECYb_558 %>% filter(sample=="REH1_PlusTet_Rep1_CYb_M1")
OEIN_KA2 <- KREH1OECYb_558 %>% filter(sample=="REH1_PlusTet_Rep2_CYb_M1")
OEIN_EQ1 <- KREH1OECYb_558 %>% filter(sample=="REH1_PlusTet_Rep1_CYb_E1")
OEIN_EQ2 <- KREH1OECYb_558 %>% filter(sample=="REH1_PlusTet_Rep2_CYb_E1")

# Use this code to assign one sample to the CYbESS24 variable at a time
...
results <- data.frame(matrix(NA, nrow = nrow(CYbESS558),
                             ncol = length(seq(1:6))))
pre <- "TAAAAGCGGAGAAAAAAGAAAGGG"
pres <- strsplit(reverse(pre),"A|G|C") %>% unlist()
nchar(pres)
preES <- nchar(pres)

for (rowIdx in 1:nrow(results)) {
  results[rowIdx, 1] <- CYbESS558[rowIdx,"junc_seq"]
  results[rowIdx, 2] <- CYbESS558[rowIdx,"norm_count"]
  results[rowIdx, 3] <- CYbESS558[rowIdx,"junc_len"]
  results[rowIdx, 4] <- CYbESS558[rowIdx,"junc_end"]
  
  # Split junction into editing sites
  s<-strsplit(reverse(CYbESS558[rowIdx,"junc_seq"]),"A|G|C") %>% unlist()
  ...
  totalpre <- sum(test[1:12])
  if(is.na(totalpre)) {totalpre <- "NA"}
  
  # Finally: check if all subset editing sites are pre-edited (equal to 12)
  if(totalpre==12) {
...
write.table(resultstest, "CYbREH1_WTOE_minTet_Rep2_ESS558_junctionanalysis.csv", sep=",", row.names=FALSE)
...
combinedtotalES558 <- combined %>% group_by(sample, kd) %>% 
...
combinedgraph <- full_join(combinedtotalES558,combinedtrue) %>%
...
ggplot(combinedgraph, aes(x=kd, y=mean, fill=kd)) +
  ...
  ggtitle("CYb gRNA1 Region Pre-edited") +
  ylab("Percent of ESS558 Reads") + xlab("Sample") +
  theme_bw() +
...
# Significance tests
WTtest1 <- combinedtotalES558 %>% filter(kd=="AvgUn" | kd=="WTOE")
WTtest2 <- WTtest1 %>% 
  group_by(kd, sample) %>% 
...
KAtest1 <- combinedtotalES558 %>% filter(kd=="AvgUn" | kd=="KAOE")
KAtest2 <- KAtest1 %>% 
  group_by(kd, sample) %>% 
  summarise(total = list(total)) %>% 
  spread(kd, total) %>% ungroup() %>%
...
EQtest1 <- combinedtotalES558 %>% filter(kd=="AvgUn" | kd=="EQOE")
EQtest2 <- EQtest1 %>% 
  group_by(kd, sample) %>% 
  summarise(total = list(total)) %>% 
  spread(kd, total) %>% ungroup() %>%
