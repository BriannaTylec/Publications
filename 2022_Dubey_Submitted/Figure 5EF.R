...
### This code contains the analysis for Figure 5E and 5F (KREH1 OE in CYb)
...
# Load the KREH1 OE CYb TREAT export file
KREH1OECYb <- read.csv(file.choose(), sep = ",", header=TRUE)
unique(KREH1OECYb$sample) #should see 12 samples
unique(KREH1OECYb$tetracycline)

# Label each sequence as Pre-edited, fully edited, or partially edited
KREH1OECYb <- KREH1OECYb %>%
...
CYb.un <- KREH1OECYb %>% filter(tetracycline=="false")
CYb.un[,4] <- "Uninduced"
CYb.pefe.un <- CYb.un %>% filter(status=='Pre' | status=='Full') %>%
  select(sample, knock_down, replicate, tetracycline, norm_count, status)
CYb.partial.un <- CYb.un %>% filter(status=='Partial') %>%
...
CYb.in <- KREH1OECYb %>% filter(tetracycline=="true")
CYb.in[CYb.in$knock_down=="REH1_WT_OE",4] <- "WT-OE"
CYb.in[CYb.in$knock_down=="REH1_K168A",4] <- "KA-OE"
CYb.in[CYb.in$knock_down=="REH1_E268Q",4] <- "EQ-OE"
CYb.pefe.in <- CYb.in %>% filter(status=='Pre' | status=='Full') %>%
  select(sample, knock_down, replicate, tetracycline, norm_count, status)
CYb.partial.in <- CYb.in %>% filter(status=='Partial') %>%
  group_by(sample) %>% mutate(norm_count=sum(norm_count)) %>%
  select(sample, knock_down, replicate, tetracycline, norm_count, status) %>%
  distinct(., .keep_all = TRUE)
...
CYbforfigure$status <- factor(CYbforfigure$status, levels=c('Pre','Partial','Full'))
...
# Significance tests #
CYb.totals <- bind_rows(CYb.totals.un, CYb.totals.in)
CYb.totals[CYb.totals$knock_down=="WT-OE",2] <- "WTOE"
CYb.totals[CYb.totals$knock_down=="KA-OE",2] <- "KAOE"
CYb.totals[CYb.totals$knock_down=="EQ-OE",2] <- "EQOE"

WTtest1 <- CYb.totals %>% filter(knock_down=="Uninduced" | knock_down=="WTOE")
WTtest2 <- WTtest1 %>% 
...
KAtest1 <- CYb.totals %>% filter(knock_down=="Uninduced" | knock_down=="KAOE")
KAtest2 <- KAtest1 %>% 
  group_by(knock_down, status) %>% 
  summarise(norm_count = list(norm_count)) %>% 
  spread(knock_down, norm_count) %>% ungroup() %>%
...
EQtest1 <- CYb.totals %>% filter(knock_down=="Uninduced" | knock_down=="EQOE")
EQtest2 <- EQtest1 %>% 
  group_by(knock_down, status) %>% 
  summarise(norm_count = list(norm_count)) %>% 
  spread(knock_down, norm_count) %>% ungroup() %>%
...
KREH1OECYb <- read.csv(file.choose(), sep = ",", header=TRUE)
unique(KREH1OECYb$sample) # Should have 12 samples
...
write.table(ESSctbyES, "CYbKREH1OEEPSforRunpu.csv", sep=",", row.names=FALSE)
###
...
# Save EPS table
write.table(EPSfromRunpu, "CYb_EPS_KREH1KO.csv", sep=",", row.names=FALSE)
###
...
ggplot() + 
  ylab("Average Norm Count") + ggtitle("CYb Exacerbated Pause Sites: KREH1 OE") +
